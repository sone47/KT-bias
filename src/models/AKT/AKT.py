...
def compute_mse(target, pred):
    return metrics.mean_squared_error(target, pred)
...
def train_one_epoch(net, params, optimizer, q_data, qa_data, pid_data):
    ...
    true_el = 0
    for idx in tqdm(range(n), "training"):
        optimizer.zero_grad()
...
def test_one_epoch(net, params, q_data, qa_data, pid_data):
    ...
    mse = compute_mse(all_target, all_pred)
    auc = compute_auc(all_target, all_pred)
    accuracy = compute_accuracy(all_target, all_pred)

    return auc, accuracy, mse
        ...
        for idx in range(epoch):
            ...
            if train_log_file:
                with open(train_log_file, 'a') as log_tf:
                    log_tf.write('{epoch},{loss: 8.5f}\n'.format(epoch=idx, loss=train_loss))

            if test_data is not None:
                _, (valid_auc, valid_accuracy, valid_mse) = self.eval(test_data)
                if valid_auc > best_auc:
                    self.save(save_filepath)
                    best_auc = valid_auc
                print("[Epoch %d] auc: %.6f, accuracy: %.6f, MSE: %.6f" % (idx, valid_auc, valid_accuracy, valid_mse))
                if test_log_file:
                    with open(test_log_file, 'a') as log_tf:
                        log_tf.write('{epoch},{auc: 8.5f},{acc:3.3f}\n'.format(epoch=idx, auc=valid_auc,
                                                                               acc=100 * valid_accuracy))

    def eval(self, test_data) -> ...:
        self.akt_net.eval()
        performance = test_one_epoch(self.akt_net, self.params, *test_data)
        return [], performance
